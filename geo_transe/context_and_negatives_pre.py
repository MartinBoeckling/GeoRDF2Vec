...
class ContextAndNegatives:
    def __init__(self, names, dataset, head_context_size, tail_context_size, negative_batch_size):
        ...
        self.names = names
        self.input_path = f"./datasets/{self.dataset}/input/"
        self.output_path = f"./datasets/{self.dataset}/output/"
        self.log_path = f"./logs/context_and_negatives_pre_on_{self.dataset}.log"
    ...
    def context_sampling(self):
        for index, _ in enumerate(self.names):
            ...
            for entity_id in trange(int(num_of_entity), desc="Context sampling"):
    ...
    def negative_sampling(self):
        # Precompute all valid negative samples for entities in advance
        all_negative_samples = self._precompute_negative_samples()

        def sample_negative_for_entity(entity, entity_dict, num_of_entity, negative_batch_size):
            negative_entities = []
            sampled_entities = set()
            sampled_entity_count = 0

            while len(negative_entities) < negative_batch_size and sampled_entity_count < num_of_entity:
                sampled_entity = random.choice(entity_dict)
                # Avoid redundant sampling
                if sampled_entity in sampled_entities:
                    continue
                sampled_entities.add(sampled_entity)
                sampled_entity_count += 1
                # Check if valid negative sample
                if self.negative_or_not(entity, sampled_entity):
                    negative_entities.append(sampled_entity)

            # If no valid negative samples found, fallback to random sampling
            if len(negative_entities) == 0:
                negative_entities = random.sample(list(entity_dict.values()), negative_batch_size)

            # If still not enough, extend the list with duplicates
            if len(negative_entities) < negative_batch_size:
                additional_samples = random.sample(negative_entities, negative_batch_size - len(negative_entities))
                negative_entities.extend(additional_samples)

            return torch.unsqueeze(torch.LongTensor(negative_entities), 0)
        ...
        with ProcessPoolExecutor(max_workers=30) as executor:
            for index, name in enumerate(self.names):
                ...
                # Using concurrent futures for parallelizing negative sampling
                futures = []
                for entity_id in trange(num_of_entity, desc="Negative Sampling"):
                    entity = entity_dict[entity_id]
                    futures.append(executor.submit(sample_negative_for_entity, entity, entity_dict, num_of_entity, self.negative_batch_size))

                # Gather results and assign to the negative dictionary
                for entity_id, future in enumerate(futures):
                    entity = entity_dict[entity_id]
                    negative[entity] = future.result()

                # Dump the negative samples to a file
                dump_data(negative, self.output_path + "%s_negatives.pickle" % name, self.log_path, "")

    def _precompute_negative_samples(self):
        # Create a precomputed set of negative samples for each entity, if applicable.
        # This can be a dictionary of entity -> list of potential negative entities
        all_negative_samples = {}
        for index, name in enumerate(self.names):
            num_of_entity = self.num_of_entities[index]
            entity_dict = self.entity_dicts[index]

            # Generate potential negative samples for each entity
            negative_samples = {}
            for entity_id, entity in entity_dict.items():
                negative_samples[entity] = random.sample(list(entity_dict.values()), self.negative_batch_size)
            all_negative_samples[self.names[index]] = negative_samples
